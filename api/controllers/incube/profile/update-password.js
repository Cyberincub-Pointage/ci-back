module.exports = {
  ...
  description: 'Mettre à jour le mot de passe de l\'incubé connecté.',
  ...
  fn: async function ({ currentPassword, newPassword }) {
    ...
    if (!incube) { throw { badCombo: 'Utilisateur introuvable.' }; }
    ...
    try {
      await Incube.updateOne({ id: this.req.me.id }).set({ password: newPassword });
    } catch (err) {
      if (err.message) {
